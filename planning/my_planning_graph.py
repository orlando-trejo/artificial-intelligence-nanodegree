...
def shared_elements(X, Y):
    """ checks if there are shared elements between two variables """
    elements = []
    shared = False
    for i in X:
        if i in Y:
...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        #Grab current state nodes
        current_state_nodes = self.s_levels[level]
        #Loop through all actions
        for action in self.all_actions:
            action_node = PgNode_a(action)
            #Check if action_node has prenodes that are contained in previous S level
            if action_node.prenodes.issubset(current_state_nodes):
                #Loop through state_nodes in current_state_nodes
                for state_node in current_state_nodes:
                    #Check that state_node is a parent of action_node
                    if state_node in action_node.prenodes:
                        state_node.children.add(action_node)
                        action_node.parents.add(state_node)
                    #Update action levels
    ...
    def add_literal_level(self, level):
        ...
        #Grab previous level action nodes
        parent_action_nodes = self.a_levels[level-1]
        #Loop through parent level action nodes
        for parent_action_node in parent_action_nodes:
            #Determine effect nodes of parent action node
            effnodes = parent_action_node.effnodes
            #Loop through effnodes
            for effnode in effnodes:
                #Track if effnode is unique
                is_unique_node = True
                #Loop through current level state nodes
                for current_state_node in self.s_levels[level]:
                    if effnode == current_state_node:
                        parent_action_node.children.add(current_state_node)
                        current_state_node.parents.add(parent_action_node)
                        is_unique_node = False
                #If effnode is unique
                if is_unique_node:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        x1 = node_a1.action.effect_add
        y1 = node_a1.action.effect_rem
        x2 = node_a2.action.effect_add
        y2 = node_a2.action.effect_rem

        return shared_elements(x1, y2) or shared_elements(x2, y1)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        x1 = node_a1.action.effect_add
        y1 = node_a1.action.effect_rem
        w1 = node_a1.action.precond_pos
        z1 = node_a1.action.precond_neg
        x2 = node_a2.action.effect_add
        y2 = node_a2.action.effect_rem
        w2 = node_a2.action.precond_pos
        z2 = node_a2.action.precond_neg

        return shared_elements(x1, z2) or shared_elements(y1, w2) or \
                shared_elements(w1, y2) or shared_elements(z1, x2)
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        x = node_s1
        y = node_s2
        return (x.symbol == y.symbol) and (x.is_pos != y.is_pos)
